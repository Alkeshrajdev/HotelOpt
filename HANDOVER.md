...
**Live vs sample (`src/lib/live/mode.tsx`).** `LIVE_ROUTES` lists the connected screens; every other route shows a "Sample data" band to a signed-in reader (`SampleDataNotice`, mounted in `AppShell`). Add a route there in the same change that wires it. The old demo-mode banner (`DemoNotice`) is no longer mounted: the owner removed it from `main` in the design-polish pass (74d3475) and that removal is kept; `SampleDataNotice` is a different thing — it never shows in the demo, only to a signed-in person on a screen that is not yet wired.

**Merged from `main` (2026-09-09).** The owner's seven design-polish commits (`bfb8a57..a664d31`: elevation, hairline borders, unified stat tiles, shorter titles, the demo banner removed, layout fixes on Portfolio Setup / AI Assistant / Data Capture, a "Custom / Other" data type) were merged into this branch; the only conflict was `AppShell.tsx` (resolved as above). Typecheck and build green after the merge.
Later merges from `main` (same day and 2026-09-10): the Generate Report layout, the Smart Ops layout pass and bar-chart height fix, the soft layered visual system (light sidebar, Plus Jakarta Sans, circular icon containers, toasts, accent lines removed), route-level code splitting, and the demo-mode client rules. Then the states pass (PageSkeleton, route ErrorBoundary, the one EmptyState), the one StatTile for every summary row, and the chart palette; the Properties registry now uses StatTile and EmptyState, with its own no-access state for a signed-in reader who holds no hotel. That last one is ported onto this branch's rewritten `supabase.ts` / `auth.tsx`: while `ho_demo` is set the client neither persists, recovers nor refreshes a session, the auth provider stops the refresh timer, and sign-out from the demo skips the remote call.
Merges on 2026-09-11 and 2026-09-12: the Smart Ops rebuild on the metering model, the "facts as pictures" pass, and "Portfolio is the only cross-property view". That last one touched the topbar wiring, so it is reconciled here: the owner's rule stands (a property-level tool always works on one property, "All Properties" exists only for Portfolio, the last-used property is remembered in `localStorage` under `ho.lastProperty`), and the list of names comes from the registry rather than the sample constant — `TopbarProvider` calls `useProperties` and exposes `propertyNames`, `propertyName` resolves only to a name in the reader's access (a remembered demo name is dropped after a real sign-in), nothing resolves while a live registry loads, and `ALL_PROPERTIES` carries no count. `EnergyOverviewLive` reads `propertyName` and no longer has a first-hotel fallback.
...
- **Extended the data model** (`propertiesData.ts`): `RichProperty` gained `floors · buildings · conditionedArea · kitchens · meetingSpaceM2 · parkingSpaces · evChargers · operatingSchedule · onSitePvKwp · climateZone · weatherStation · legalEntity · registrationNo · meters[]` (new `Meter`/`MeterType`/`MeterCapture` types). Populated for all 10 hotels via `FACILITY` + `METERS` maps merged into a `BaseProperty` (Omit) array — real typed fields, distinct per hotel (e.g. Cape Town has a diesel standby genset for load-shedding; Zermatt is seasonal; Dubai has Empower district cooling).
- **Rebuilt the Configuration tab** (`PropertyDetail.tsx`): enriched sections — Identity & legal (legal entity, registration), Location & climate (climate zone, weather station), Facility & capacity (conditioned area, floors/buildings, kitchens, meeting space, parking, EV chargers, on-site PV), Operations & reporting. New **editable Meters & utility accounts registry** (Add/Edit/remove via `MeterModal`, local state) that backs the Data Capture `meterId` field. New **"Configured elsewhere" cross-links** (Targets → Portfolio Setup, Frameworks & GHG boundary → Portfolio Setup, Emission factors → Admin, Operational drivers & data sources → Data Capture) so the page orients without duplicating.
- **Deferred to Portfolio Setup (client will do):** the B′ gap — reporting frameworks (GHG/GRI/CSRD/SBTi/CDP), GHG org boundary, consolidation approach, GWP set, Scope 2 method as real config (currently hardcoded in the GHG Inventory report).
- tsc clean; verified live (sections render, per-property data distinct, Add-meter flow works, no console errors — earlier `PROPERTIES` HMR errors were a transient mid-edit state).

### Session 2026-06-23 (cont.) — Guest Engagement: design rework + new features (DONE, verified)
Reworked the Guest Engagement page (`pages/GuestEngagement.tsx`) — design + new features (front-end).
- **Property selector** (header) scoping the page to one of the canonical 10 hotels (resolves the long-deferred "Guest Engagement is hardcoded to Skyline Dubai with no picker"). Threads into the Public-page title/domain-slug/hero/region, the Surveys title, and the Campaigns default + property options. Verified: switching to "The Montrose Paris" updates the public title, `the-montrose-paris.hotel-optimizer.com`, and the hero.
- **Tabs migrated** to the shared `Tabs` primitive; "Open public page" now jumps to the Public tab.
- **New default "Overview" tab** with three added features:
  - **Engagement pulse** — 5 clickable KPI tiles (Public page status · guest reach 30d · NPS · eco-actions 30d · QR scans 30d) that deep-jump to the relevant tab.
  - **Collective guest impact** (the star widget) — aggregates anonymised guest eco-actions into a shareable narrative: "Together, our guests saved 4.2 t CO₂e / 85 m³ water / 88 kg waste this month", with a per-action breakdown and a "Share impact" pack button. Derived from per-action impact factors × monthly counts (`GUEST_ACTIONS`).
  - **Guest journey touchpoint map** — pre-stay → in-stay → post-stay, showing where sustainability reaches the guest and which touchpoints are live vs not-configured (e.g. "Guest review request — set up").
- tsc clean; verified live (property scoping, pulse navigation, mobile 375px, no console errors). Removed dead imports (BarChart2/Minus/GUEST_PAGE_METRICS).

**Follow-on — four more GE features (DONE, verified):**
- **Eco-points → rewards & challenges**: added a **Rewards catalogue** (what points redeem for, with redemptions), **Active challenges** (bonus-point eco-streaks with completion %), and **Guest loyalty tiers** (Sprout→Evergreen distribution bar) to the Eco-points tab.
- **Campaign performance funnel**: campaign rows are now clickable → a detail modal with a **Sent → Delivered → Opened → Clicked → Eco-action** conversion funnel (per-step % + step-conversion), tied back to the collective-impact total. Edit button stops propagation.
- **NPS & sentiment trend** on Surveys: a 6-month NPS line (inline `Trend` SVG) + per-month sentiment-mix bars + "+14 pts since Nov" badge.
- **New "QR Points" tab**: scan analytics — tiles (scans 30d = 2,140, scan→action %, active points, unique guests), an 8-week scan-volume trend, and a per-point breakdown (location · destination · scans · conversion). Per-point scans reconcile to the pulse's 2,140.
- Added a lightweight reusable inline `Trend` (area+line SVG, no chart lib) used by Surveys + QR. tsc clean; all four verified live; no console errors.
...
- **Tab-strip consistency.** Migrated the prominent hand-rolled `border-b-2` tab strips to the shared `Tabs` primitive: `PropertyDetail` (6 property sections), `ReviewApproval` (Approval Queue / Capture Status / Platform Review), and `Dashboard` (Overview / Environment / Targets / Hotels / Social & Governance). Together with the Performance hub (part 1) that's all four prominent strips on one accessible primitive + the global focus ring.
- **Remaining in this pass (deferred):** section anchors / sticky sub-nav on the long scroll pages (Environment dashboard tab, Smart Ops) — the review ranked this low ("orientation already improved"), so skipped for now. Smart Ops still has its own bespoke tab/nav chrome → fold into the visual-consistency pass.
