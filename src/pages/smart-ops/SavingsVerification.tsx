  ...
  return (
    ...
    >
      <div className="space-y-5">
        {/* Status timeline */}
        <div>
          <h4 className="text-xs font-semibold text-ink-500 uppercase tracking-wider mb-3">Verification Timeline</h4>
                ...
                <div key={step} className="flex items-center gap-2 flex-1 min-w-0">
                  <div className="flex flex-col items-center flex-shrink-0">
                    <div className={cn(
                      ...
                      isCurrent ? "bg-info text-white ring-info" :
                      isReached ? "bg-good text-white ring-good" :
                      "bg-ink-100 text-ink-400 ring-ink-100"
                    )}>
                      {isReached && !isCurrent ? <CheckCircle className="w-4 h-4" /> : i + 1}
                    </div>
                    <div className="text-[10px] text-ink-500 mt-1 text-center leading-tight">{step}</div>
                    {dateMap[step] && (
                      <div className="text-[10px] text-ink-400 text-center">{dateMap[step]}</div>
                    )}
                  </div>
                  {i < arr.length - 1 && (
                    <div className={cn("flex-1 h-0.5 mb-4", isReached && stepIdx < currentIdx ? "bg-good" : "bg-ink-200")} />
                  )}
                </div>
              );
            })}
        ...
        <div>
          <h4 className="text-xs font-semibold text-ink-500 uppercase tracking-wider mb-3">Baseline vs Post-Action Consumption</h4>
          <div className="bg-ink-50 rounded-xl p-4 space-y-3">
            ...
            <div className="pt-2 border-t border-ink-200">
              <div className="flex justify-between text-sm">
                <span className="text-ink-600 font-medium">Verified Saving</span>
                <span className="font-bold text-good-700">{saving.verifiedSaving}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Financial & Carbon */}
        <div>
          <h4 className="text-xs font-semibold text-ink-500 uppercase tracking-wider mb-3">Financial & Carbon Impact</h4>
          <div className="grid grid-cols-3 gap-3">
            <div className="bg-good/10 border border-good/30 rounded-xl p-4">
              <div className="text-xs text-good-700 mb-1">Cost Saving</div>
              <div className="text-lg font-bold text-good-700">{saving.costSaving}</div>
            </div>
            <div className="bg-info/10 border border-info/30 rounded-xl p-4">
              <div className="text-xs text-info-700 mb-1">Carbon Saving</div>
              <div className="text-lg font-bold text-info-700">{saving.carbonSaving || "—"}</div>
            </div>
            <div className="bg-ink-50 border border-ink-200 rounded-xl p-4">
              <div className="text-xs text-ink-500 mb-1">Confidence Level</div>
        ...
        {saving.normalisationNote && (
          <div>
            <h4 className="text-xs font-semibold text-ink-500 uppercase tracking-wider mb-3">Normalisation Method</h4>
            <div className="p-4 bg-info/10 border border-info/30 rounded-xl flex gap-2">
              <Info className="w-4 h-4 text-info-700 flex-shrink-0 mt-0.5" />
              <p className="text-sm text-info-700">{saving.normalisationNote}</p>
            </div>
          </div>
        )}
        ...
        {/* Evidence */}
        <div>
          <h4 className="text-xs font-semibold text-ink-500 uppercase tracking-wider mb-3">Evidence Documents</h4>
          <div className="rounded-lg border border-ink-200 overflow-hidden">
            <div className="px-4 py-3 flex items-center justify-between bg-ink-50">
              <span className="text-sm text-ink-600">Evidence status</span>
            ...
            {saving.evidenceStatus === "Attached" ? (
              <div className="divide-y divide-ink-100">
                <div className="px-4 py-2.5 flex items-center justify-between text-sm">
                  <span className="text-ink-700">Meter data export (baseline period)</span>
                  <button className="text-xs text-info-700 hover:underline">View</button>
                </div>
                <div className="px-4 py-2.5 flex items-center justify-between text-sm">
                  <span className="text-ink-700">Meter data export (post-action period)</span>
                  <button className="text-xs text-info-700 hover:underline">View</button>
                </div>
                <div className="px-4 py-2.5 flex items-center justify-between text-sm">
                  <span className="text-ink-700">Verification sign-off — Sustainability Team</span>
                  <button className="text-xs text-info-700 hover:underline">View</button>
                </div>
              </div>
            ) : (
        ...
        {/* Source alert link */}
        {saving.sourceAlert && (
          <div className="p-3 bg-warn/10 border border-warn/30 rounded-lg flex items-center justify-between">
            <span className="text-sm text-warn-700">Created from fault alert: <span className="font-mono font-semibold">{saving.sourceAlert}</span></span>
            <button className="text-xs text-warn-700 hover:underline flex items-center gap-1">
...
function NormalisationInfoCard() {
  ...
  return (
    <div className="rounded-xl border border-ink-200 overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          <Info className="w-4 h-4 text-info-700" />
          <span className="text-sm font-medium text-ink-700">Normalisation Methods Explained</span>
        </div>
        <ChevronRight className={cn("w-4 h-4 text-ink-400 transition-transform", expanded && "rotate-90")} />
      </button>
      {expanded && (
        <div className="px-4 py-4 grid grid-cols-1 md:grid-cols-2 gap-3 bg-white">
          {[
            ...
            <div key={method} className="p-3 bg-ink-50 rounded-lg">
              <div className="text-sm font-semibold text-ink-700 mb-1">{method}</div>
          ...
          <table className="w-full text-sm min-w-[960px]">
            <thead>
              <tr className="border-b border-ink-100">
                ...
                <th className="text-left px-4 py-3 text-xs text-ink-500 font-medium">Category</th>
                <th className="text-left px-4 py-3 text-xs text-ink-500 font-medium">System / Asset</th>
                <th className="text-left px-4 py-3 text-xs text-ink-500 font-medium">Status</th>
                <th className="text-left px-4 py-3 text-xs text-ink-500 font-medium">Baseline</th>
                <th className="text-left px-4 py-3 text-xs text-ink-500 font-medium">Post-Action</th>
                <th className="text-left px-4 py-3 text-xs text-ink-500 font-medium">Verified Saving</th>
                <th className="text-left px-4 py-3 text-xs text-ink-500 font-medium">Cost Saving</th>
                <th className="text-left px-4 py-3 text-xs text-ink-500 font-medium">Carbon</th>
                <th className="text-left px-4 py-3 text-xs text-ink-500 font-medium">Confidence</th>
                <th className="text-left px-4 py-3 text-xs text-ink-500 font-medium">Evidence</th>
                <th className="text-left px-4 py-3 text-xs text-ink-500 font-medium">Action</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-ink-50">
              {records.map((saving) => (
                <tr key={saving.id} className="hover:bg-ink-50 transition-colors">
                  <td className="px-4 py-3">
                    <div className="font-medium text-ink-800 max-w-52">{saving.name}</div>
                    <div className="text-xs text-ink-400 mt-0.5">{saving.id}</div>
                    {saving.sourceAlert && (
                      <div className="text-xs text-warn-700 mt-0.5">← {saving.sourceAlert}</div>
                    )}
                  </td>
                  <td className="px-4 py-3">
                    <span className={cn(
                      "text-xs font-medium",
                      saving.category === "Energy" ? "text-warn-700" :
                      saving.category === "Water" ? "text-info-700" :
                      "text-good-700"
                    )}>
                      {saving.category}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-ink-600 text-xs max-w-32">{saving.systemAsset}</td>
                  <td className="px-4 py-3">
                    <Badge tone={statusTone(saving.status)}>{saving.status}</Badge>
                  </td>
                  <td className="px-4 py-3 text-ink-500 text-xs max-w-28">{saving.baselineConsumption}</td>
                  <td className="px-4 py-3 text-ink-500 text-xs max-w-28">{saving.postActionConsumption}</td>
                  <td className="px-4 py-3 font-medium text-ink-800 text-xs">{saving.verifiedSaving}</td>
                  <td className="px-4 py-3">
                    {saving.status === "Verified" || saving.status === "Reported" ? (
                      <span className="text-good-700 font-semibold text-xs">{saving.costSaving}</span>
                    ) : (
                      <span className="text-warn-700 text-xs">{saving.costSaving}</span>
                    )}
                  </td>
                  <td className="px-4 py-3 text-ink-600 text-xs">{saving.carbonSaving || "—"}</td>
                  <td className="px-4 py-3">
                    <Badge tone={confidenceTone(saving.confidenceLevel)}>{saving.confidenceLevel}</Badge>
                  </td>
                  <td className="px-4 py-3">
              ...
              {records.length === 0 && (
                <tr>
                  <td colSpan={12} className="px-4 py-12 text-center text-ink-400 text-sm">
  ...
  return (
    <div className="bg-ink-50 rounded-xl p-4">
      ...
      <div className="flex gap-0.5 mt-1">
        {data.map((d) => (
          <div key={d.month} className="flex-1 text-center text-[10px] text-ink-400 truncate">
      ...
      <div className="flex items-center gap-6 mt-3 text-xs text-ink-500">
        ...
        <span className="ml-auto text-ink-400">Values in {unit}</span>
      </div>
    </div>
  );
}
...
function SavingsPipelineStrip() {
  ...
  return (
    <div className="flex items-stretch gap-0 rounded-xl overflow-hidden border border-ink-200">
      {pipelineSteps.map((step, i) => (
        <div key={step.label} className="flex items-center flex-1 min-w-0">
          <div className="flex-1 p-4 bg-white">
            ...
            <div className="text-2xl font-bold text-ink-800">{step.count}</div>
            <div className="text-xs text-ink-400 mt-0.5">saving{step.count !== 1 ? "s" : ""}</div>
          </div>
...
function EnergySavingsTab() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <div className="p-4">
          <div className="text-sm font-semibold text-ink-700 mb-1">Monthly Energy Savings — Verified vs Estimated</div>
          <div className="text-xs text-ink-400 mb-4">Apr 2025 – May 2026 · Values in MWh</div>
          <DualBarChart
            data={MONTHLY_ENERGY}
...
function WaterSavingsTab() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <div className="p-4">
          <div className="text-sm font-semibold text-ink-700 mb-1">Monthly Water Savings Trend</div>
          <div className="text-xs text-ink-400 mb-4">Apr 2025 – May 2026 · Values in m³</div>
          <DualBarChart
            data={MONTHLY_WATER}
            leftLabel="Verified savings"
            leftColour="bg-info"
...
const REPORTING_CATEGORIES = [
  ...
  },
  {
    ...
    colour: "bg-ink-50 border-ink-200",
    headerColour: "text-ink-700 bg-ink-100",
  },
  {
    status: "Reported",
    ...
    <div className="space-y-6">
          ...
          return (
            <div key={cat.status} className={cn("rounded-xl border p-5 space-y-3", cat.colour)}>
              <div className="flex items-center justify-between">
                <h3 className="font-semibold text-ink-800">{cat.label}</h3>
                <Badge tone={cat.tone}>{records.length} saving{records.length !== 1 ? "s" : ""}</Badge>
              </div>
              <div className="space-y-2">
                <div className="flex items-start gap-2">
                  <span className="text-xs text-ink-500 w-28 flex-shrink-0 pt-0.5">Report-ready:</span>
                  <span className={cn("text-xs font-medium", cat.reportReady.startsWith("Yes") ? "text-good-700" : "text-ink-600")}>
                    {cat.reportReady}
                  </span>
                </div>
                <div className="flex items-start gap-2">
                  <span className="text-xs text-ink-500 w-28 flex-shrink-0 pt-0.5">Frameworks:</span>
                  <span className="text-xs text-ink-600">{cat.frameworks}</span>
                </div>
              </div>
              <div className="p-3 bg-white bg-opacity-60 rounded-lg">
                <p className="text-xs text-ink-600 leading-relaxed">{cat.notes}</p>
              </div>
              {records.length > 0 && (
                <div className="space-y-1">
                  {records.slice(0, 4).map((r) => (
                    <div key={r.id} className="flex items-center gap-1.5 text-xs text-ink-600">
                      <ChevronRight className="w-3 h-3 text-ink-400 flex-shrink-0" />
                      {r.name}
                    </div>
                  ))}
                  {records.length > 4 && (
                    <div className="text-xs text-ink-400 pl-4">+{records.length - 4} more</div>
                  )}
                </div>
              )}
            </div>
      ...
      <Card>
        <div className="p-4 border-b border-ink-100">
          <div className="text-sm font-semibold text-ink-700">Reporting Eligibility Summary</div>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-ink-100">
                <th className="text-left px-4 py-3 text-xs text-ink-500 font-medium">Saving</th>
                <th className="text-left px-4 py-3 text-xs text-ink-500 font-medium">Status</th>
                <th className="text-left px-4 py-3 text-xs text-ink-500 font-medium">Report-Ready</th>
                <th className="text-left px-4 py-3 text-xs text-ink-500 font-medium">Framework Eligibility</th>
                <th className="text-left px-4 py-3 text-xs text-ink-500 font-medium">Notes</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-ink-50">
              {SAVINGS.map((saving) => {
                ...
                return (
                  <tr key={saving.id} className="hover:bg-ink-50 transition-colors">
                    <td className="px-4 py-3">
                      <div className="font-medium text-ink-800 text-xs max-w-52">{saving.name}</div>
                      <div className="text-[11px] text-ink-400">{saving.id}</div>
                    </td>
                    <td className="px-4 py-3">
                      <Badge tone={statusTone(saving.status)}>{saving.status}</Badge>
                    </td>
                    ...
                    <td className="px-4 py-3 text-xs text-ink-500 max-w-52">
      ...
      <div className="p-5 bg-warn/10 border border-warn/30 rounded-xl">
        <div className="flex gap-3">
          <AlertTriangle className="w-5 h-5 text-warn-700 flex-shrink-0 mt-0.5" />
          <div>
            <p className="text-sm font-semibold text-warn-700 mb-1">Reporting Eligibility Rule</p>
      ...
      <div className="flex items-center justify-between p-4 bg-info/10 border border-info/30 rounded-xl">
        <div className="flex items-center gap-3">
          <FileCheck className="w-5 h-5 text-info-700" />
          <div>
            <div className="text-sm font-semibold text-info-700">Ready to include verified savings in a report?</div>
            <div className="text-xs text-info-700 mt-0.5">Navigate to the Reports module to build your sustainability report.</div>
          </div>
        </div>
        <button className="inline-flex items-center gap-2 px-4 py-2 bg-info text-white text-sm font-medium rounded-lg hover:bg-info-700 transition-colors">
  ...
  return (
    <div>
      <div className="space-y-5">
        ...
        <div className="flex gap-3 p-4 bg-warn/10 border border-warn/30 rounded-xl">
          <AlertTriangle className="w-5 h-5 text-warn-700 flex-shrink-0 mt-0.5" />
          <p className="text-sm text-warn-700 leading-relaxed">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          />
          <KpiTile
            icon={<TrendingUp className="w-5 h-5 text-warn-700" />}
            iconBg="bg-warn/10"
          ...
          />
          <KpiTile
            icon={<Clock className="w-5 h-5 text-info-700" />}
            iconBg="bg-info/10"
          ...
          />
          <KpiTile
            icon={<Zap className="w-5 h-5 text-ink-600" />}
            iconBg="bg-ink-100"
